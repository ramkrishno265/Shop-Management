...
import { 
  FiPlus, FiSearch, FiPackage, FiBarChart2, 
  FiAlertTriangle, FiSlash, FiEdit2, FiTrash2, FiLoader 
} from 'react-icons/fi';
...
const InventoryPage = () => {
  ...
  const handleDelete = async (id) => {
    ...
    try {
      ...
      if (response.ok) {
        ...
        alert("পণ্য ডিলিট করতে সমস্যা হয়েছে।");
      }
    } catch (error) {
  ...
  const outOfStockCount = products.filter(p => p.quantity === 0 && (!p.packs || p.packs.length === 0)).length;
  const lowStockCount = products.filter(p => p.quantity > 0 && p.quantity <= (p.lowStockLimit || 5)).length;
    ...
    <div className="min-h-screen bg-slate-50/70 p-4 md:p-8 font-sans">
      <div className="max-w-7xl mx-auto space-y-6">
        ...
        <div className="flex flex-col md:flex-row md:items-center justify-between gap-4">
          ...
          {/* Add Product Button (Navigates to /product_entry) */}
          <button 
            onClick={() => navigate('/product_entry')}
            className="px-5 py-2.5 bg-slate-900 hover:bg-slate-800 text-white font-semibold text-sm rounded-xl shadow-md shadow-slate-900/10 flex items-center justify-center gap-2 transition"
          >
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4">
          ...
          <div className="bg-white p-5 rounded-2xl border border-slate-200/80 shadow-sm flex items-center justify-between">
            <div>
              ...
              <h3 className="text-2xl font-extrabold text-slate-800 mt-1">{totalProductsCount}</h3>
            </div>
          ...
          <div className="bg-white p-5 rounded-2xl border border-slate-200/80 shadow-sm flex items-center justify-between">
            <div>
              ...
              <h3 className="text-2xl font-extrabold text-slate-800 mt-1">{outOfStockCount}</h3>
            </div>
        ...
        <div className="bg-white p-4 rounded-2xl border border-slate-200/80 shadow-sm flex flex-col md:flex-row items-center gap-3">
          ...
          <div className="w-full md:w-48">
            ...
            >
              ...
              {/* চাইলে ক্যাটাগরি ডাইনামিক করতে পারেন */}
        ...
        <div className="bg-white rounded-2xl border border-slate-200/80 shadow-sm overflow-hidden">
            ...
            <table className="w-full text-left border-collapse min-w-[850px]">
              ...
              <tbody className="divide-y divide-slate-100 text-sm">
                {loading ? (
                  ...
                  products
                    ...
                    .map((product) => (
                      <tr key={product.id} className="hover:bg-slate-50/50 transition">
                        ...
                        <td className="p-4 text-slate-600">{product.category?.name || product.category || 'N/A'}</td>
                        <td className="p-4 text-slate-600">৳{product.purchasePrice?.toFixed(2) || '0.00'}</td>
                        <td className="p-4 text-slate-600">৳{product.sellingPrice?.toFixed(2) || '0.00'}</td>
                        
                        {/* Stock Quantity Column (Handles both Standard & Packs) */}
                        <td className="p-4">
                          {product.inventoryType === 'pack' && product.packs && product.packs.length > 0 ? (
                            <div className="space-y-0.5 text-xs">
                              {product.packs.map((pk, idx) => (
                                <div key={idx} className="text-indigo-600 font-medium">
                                  {pk.stock} {pk.packName}: <span className="text-slate-500">({pk.stock * pk.multiplier} Pcs)</span>
                                </div>
                              ))}
                            </div>
                          ) : (
                            <span className="font-semibold text-slate-800">{product.quantity} {product.baseUnit || 'Pcs'}</span>
                          )}
                        </td>
                  ...
                  <tr>
                    <td colSpan="8" className="text-center py-10 text-slate-400 font-medium">
                      কোনো পণ্য পাওয়া যায়নি! ওপরের "+ Add Product" বাটন থেকে পণ্য যোগ করুন।
                    </td>
                  </tr>
                )}
              </tbody>
          ...
          <div className="p-4 bg-slate-50/50 border-t border-slate-200 flex flex-col sm:flex-row items-center justify-between text-xs text-slate-500 gap-3">
            ...
            <div className="flex items-center gap-2">
              <button disabled className="px-3 py-1.5 bg-white border border-slate-200 rounded-lg text-slate-400 font-semibold cursor-not-allowed">Previous</button>
              <span className="px-3 py-1.5 font-bold text-slate-700 bg-white border border-slate-200 rounded-lg">Page 1 of 1</span>
              <button disabled className="px-3 py-1.5 bg-white border border-slate-200 rounded-lg text-slate-400 font-semibold cursor-not-allowed">Next</button>
            </div>
          </div>
        </div>

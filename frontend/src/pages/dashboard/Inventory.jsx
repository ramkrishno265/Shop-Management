          ...
          <div className="bg-white p-5 rounded-2xl border border-slate-200/80 shadow-sm flex items-center justify-between">
            <div>
              ...
              <h3 className="text-2xl font-extrabold text-slate-800 mt-1">
                {products.reduce((acc, curr) => acc + calculateTotalStock(curr), 0)}
              </h3>
            </div>
                ...
                {loading ? (
                  ...
                  products
                    ...
                    .map((product) => {
                      ...
                      const isPack = product.inventoryType === 'pack' && product.packs && product.packs.length > 0;
                        ...
                        <tr key={product.id} className="hover:bg-slate-50/50 transition">
                          ...
                          <td className="p-4">
                            {isPack ? (
                              ...
                              >
                                <span>{totalStock} {product.baseUnit || 'Pcs'}</span>
                                <span className="text-[10px] bg-violet-200 text-violet-800 px-1.5 py-0.5 rounded-md">ডিটেইলস</span>
                              </button>
                            ) : (
                  ...
                  <tr>
                    <td colSpan="8" className="text-center py-10 text-slate-400 font-medium">
                      কোনো পণ্য পাওয়া যায়নি! ওপরের "+ Add Product" বাটন থেকে পণ্য যোগ করুন।
                    </td>
                  </tr>
                )}
              </tbody>
          ...
          <div className="bg-white rounded-3xl max-w-md w-full p-6 shadow-2xl border border-slate-100 animate-in fade-in zoom-in duration-200">
            ...
            <div className="space-y-3 max-h-60 overflow-y-auto pr-1">
              {selectedProductPacks.packs.map((pack, idx) => {
                const totalBaseUnits = (Number(pack.stock) || 0) * (Number(pack.multiplier) || 1);
                return (
                  <div key={idx} className="bg-slate-50 p-3.5 rounded-2xl border border-slate-200/60 flex justify-between items-center">
                    <div>
                      <h4 className="font-bold text-slate-800 text-sm">{pack.packName || 'Default Pack'}</h4>
                      <p className="text-xs text-slate-500 mt-0.5">
                        স্টক: {pack.stock} প্যাক (প্রতিটিতে {pack.multiplier} {selectedProductPacks.baseUnit || 'Pcs'})
                      </p>
                    </div>
                    <div className="text-right">
                      <span className="font-extrabold text-violet-600 text-sm">
                        {totalBaseUnits} {selectedProductPacks.baseUnit || 'Pcs'}
                      </span>
                    </div>
                  </div>
                );
